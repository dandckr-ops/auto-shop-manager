...
The app includes built-in Keycloak/OIDC login plus optional local
...
```text
Internet -> Cloudflared tunnel -> Auto Shop Manager -> Postgres
```
...
Set `POSTGRES_PASSWORD` in `.env.sso` before starting the stack. The OIDC
values in `.env.sso` are bootstrap values; after the app starts, you can edit
auth settings from the Settings page. Do not commit real `.env` or `.env.sso`
files.
...
- Built-in local login and Keycloak/OIDC login
